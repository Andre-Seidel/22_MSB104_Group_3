...
Nama_10r_3gdp contains GDP data on nuts3 level for many countries and contains multiple columns.
The values we are looking for seems to be in the column named OBS_VALUE and the region is in a column named geo.
...
demo_r\_pjanaggr3 contains population data on nuts3 level and contains similar coulmns as the other file with the same names for value, region and year.
This file also have multiple values with different groupings.
There is a column named age for age groups.
We have chosen rows there age is TOTAL.
There is also another column for sex and we have chosen rows where sex == T which looks like the total value for both males and females.
...
We now have a dataset with combined GDP and population values.
It looks like this:
