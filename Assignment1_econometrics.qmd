...
lifexp <- read_csv('DEMO_R_MLIFEXP.csv') %>%
  ...
  filter(unit=="YR" & age=="Y_LT1" & sex=="T" & (grepl('DE..', geo) | grepl('FI..', geo) | grepl('RO..', geo))) %>%
...
education <- read_csv('EDAT_LFSE_04.csv') %>%
  ...
  filter(unit=="PC" & isced11=="ED5-8" & age=="Y25-64" & sex=="T" & (grepl('DE..', geo) | grepl('FI..', geo) | grepl('RO..', geo))) %>%
...
transport <- read_csv('TRAN_R_NET.csv') %>%
  ...
  filter(unit=="KM_TKM2" & tra_infr=="MWAY" & (grepl('DE..', geo) | grepl('FI..', geo) | grepl('RO..', geo))) %>%
...
#### Estimate the effect of regional development on regional inequality for the year 2010

#### Discuss the goodness of fit of your estimated model.

#### Plot the relationship between regional development and regional inequality and the fitted line corresponding to your estimate.
...
# The level of economic development is measured by the #log of the real GDP p.c. in constant U.S. $ in
#power-purchasing parities (PPP)
data <- data %>%
...
lm_gdppc <- lm(gini ~ log_gdppc, data = germany2010)
...
Life expecancy age in age (demographics category) Amount of motorway in kilometers (transport infrastructure category) Amount of persons with higher education (education category)
...
finland2010 <- data %>%
    filter(Year==2010 & nuts0=="FI") 

romania2010 <- data %>%
    filter(Year==2010 & nuts0=="RO") 


multi_linear_regression_model_1 <- function(mlrdata){
lm.4 = lm(gini ~ eduval , mlrdata) 
lm.5 = lm(gini ~  motorway, mlrdata)
lm.6 = lm(gini ~ eduval + motorway, mlrdata)
#Gen Table
  huxreg(
  list("Model 1" = lm.4, "Model 2" = lm.5, "Model 3" = lm.6),
  statistics = c(N = "nobs", R2 = "r.squared"),
  coefs = c(education = "eduval","motorway", const. = "(Intercept)"),
  note = "Note: {stars} T statistics in brackets."
  )
}

multi_linear_regression_model_2 <- function(mlrdata){
lm.4 = lm(gini ~ eduval , mlrdata) 
lm.5 = lm(gini ~  lifeexpectancyage, mlrdata)
lm.6 = lm(gini ~ eduval + lifeexpectancyage, mlrdata)
#Gen Table
  huxreg(
  list("Model 1" = lm.4, "Model 2" = lm.5, "Model 3" = lm.6),
  statistics = c(N = "nobs", R2 = "r.squared"),
  coefs = c(education = "eduval","lifeexpectancyage", const. = "(Intercept)"),
  note = "Note: {stars} T statistics in brackets."
  )
}
```
...
Estimate multiple linear regression model for germany on the effect education and motorway has on gini

```{r}
multi_linear_regression_model_1(germany2010)
```

Estimate multiple linear regression model for germany on the effect education and life expectancy age has on gini

```{r}
multi_linear_regression_model_2(germany2010)
```

#### Finland

Estimate multiple linear regression model for Finland on the effect education and motorway has on gini

```{r}
multi_linear_regression_model_1(finland2010)
```

Estimate multiple linear regression model for Finland on the effect education and life expectancy age has on gini

```{r}
multi_linear_regression_model_2(finland2010)
```

#### Romania

Estimate multiple linear regression model for Romania on the effect education and motorway has on gini

```{r}
multi_linear_regression_model_1(romania2010)
```

Estimate multiple linear regression model for Romania on the effect education and life expectancy age has on gini

```{r}
multi_linear_regression_model_2(romania2010)
```
