...
We begin the work on this assignment by loading relevant datasets.
In this task we will use the packages mentioned above: Nama_10_3gdp and Demo_r\_pjanaggr3.
Below we show the codes we have used to load the data.
...
Here we can see that one region have a much higher result then the rest.
We also see that some regions have a zero value.
...
However the max value for Ginie in this country is only 0.379 and the differences are not that big.
