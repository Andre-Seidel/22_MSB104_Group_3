...
Nama_10r_3gdp contains GDP data on nuts3 level for many countries and contains multiple columns. The values we are looking for seems to be in the column named OBS_VALUE and the region is in a column named geo. There is also a column for year named TIME_PERIOD.

There are different types of GDP values and the unit is stored in a column named unit. We have chosen to use values where unit is MIO_EUR. 

demo_r\_pjanaggr3 contains population data on nuts3 level and contains similar coulmns as the other file with the same names for value, region and year. This file also have multiple values with different groupings. There is a column named age for age groups. We have chosen rows there age is TOTAL. There is also another column for sex and we have chosen rows where sex == T which looks like the total value for both males and females.
...
We now have a dataset with combined GDP and population values. It looks like this:
...
ginidata <- gdpdata %>%
  ...
  filter(unit =='MIO_EUR' & (grepl('DE...', Country) | grepl('FI...', Country) | grepl('RO...', Country))) %>%
...
The new data is a table with three columns, nuts2 string, Year and the gini calculation. All three countries are in the same table and contains 1020 rows.
...
For Germany there are at least two regions that stick out from the rest when looking at the plot. They are difficult to identify graphically, but we can check the data to find those with zero values.
...
This gives two candidate regions as outliers. At least FI20 looks like it is zeros for all years.
