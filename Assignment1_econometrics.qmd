...
Since we now have GDP per capita for each region, for each year, we can for example find the wealthiest and least wealthiest regioens for a given year. We chose 2019 to investigate our results.
...
The plot "GDP Plot" shows the three countries' GDP without taking into account the population in the country.
In this plot we see that Finland and Romania are relatively close, while there is a large gap up to Germany's level of gdp.
The countries are of different sizes, and the population is a factor of central importance for the production of gdp as shown in the previous plot.
...
In order to be able to compare the countries' gdp and get a realistic representation, it is central which factors are included in the plot.
There are clear differences in the representation of GDP in the two plots shown above.
the countries are placed differently, depending on which factors are included in the coding.
...
By sorting the data shown in the table above, we can present a more clear presentation of the data for each of the three countries included.
