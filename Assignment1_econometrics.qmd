...
@tbl-mlr-germany shows that R2 is highest for model 3 which is a combination of education and motorway. Model 2 which is motorway alone gives a very low R2.
...
As we already mentioned the data set for Finland only gives us 3 values/regions for 2010. @tbl-mlr-finland shows a high value for R2 with model 1,3,4 and 5 however we cant conclude anything useful with so little data.
...
We can reject H0 since t is larger than tcrit. This means that education have an effect on regional inequality.
...
We can not reject H0 since t is smaller than tcrit. This means that motorway does not have an effect on regional inequality.
...
We can reject H0 since t is larger than tcrit. This means that life expectancy age have an effect on regional inequality.
