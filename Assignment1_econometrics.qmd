...
We can see that there are some values for Gini that is 0. We choose to filter them out as they probably are caused by missing data.
...
By using data for the year 2010 on our selected countries, we estimate the effect of regional development on regional inequality by using the simple regression model.
...
Coefficient of determination R2 is very high for Finland. However Finland has very few data points available for 2010 with only 3 values. If we combine the data from all three countries R2 becomes almost zero and there seems to be no good fit for our estimate. For germany we get R2 = 0.376 and Romania is close with R2 = 0.361. If we assume that a randomized control trial usually will give R2 < 0.1 at our estimate is a lot better than that.
...
Life expectancy age in years (demographics category) Amount of motorway in kilometers per thousand square kilometers. (transport infrastructure category)
Amount of persons with higher education in percent (education category)
